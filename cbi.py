...
def make_prediction(batch, model):
    ...
    prob = dict()
    prob["ben"] = outputs[0][0].item()
    prob["mal"] = outputs[0][1].item()
    return predicted, labels.item(), prob
...
def main(args):
    ...
    for model_path in models_to_explain:
        ...
        for aug_nr, aug_type in enumerate(augmentation_names):
            ...
            # biased data
            _, biased_test_transform = get_transforms(
            ...
            _, clean_test_transform = get_transforms(
                img_size, type_aug=aug_type, mask_dir=mask_dir, aug_p=0.0)
